...
create view if not exists v_worst_sessions as
select s.id, s.title, s.project_dir, s.agent,
       count(distinct t.id) as turns,
       count(f.id) as frictions,
       sum(case f.severity when 'high' then 3 when 'medium' then 2 else 1 end) as score,
       sum(case when t.outcome='interrupted' then 1 else 0 end) as interrupted_turns,
       max(s.last_seen) as last_seen
from session s
left join turn t on t.session_id = s.id
left join friction f on f.session_id = s.id
group by s.id
order by score desc nulls last, frictions desc;

create view if not exists v_tool_error_rates as
select tool,
       count(*) as calls,
...
create view if not exists v_harness_fixes as
...
// A CTE can wrap a mutating statement (`with x as (...) delete ...`), so also reject write keywords outright.
const WRITE_KEYWORD = /\b(insert|update|delete|drop|alter|create|replace|attach|detach|pragma|vacuum|reindex)\b/i;
...
export function openDb(path: string, options: { readonly?: boolean } = {}) {
  ...
  const api = {
    ...
    readonlyQuery(sql: string): Record<string, unknown>[] {
      ...
      if (WRITE_KEYWORD.test(sql)) throw new Error("Only SELECT / WITH statements are allowed (write keyword found)");
